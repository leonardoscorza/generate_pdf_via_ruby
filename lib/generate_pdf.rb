...
module GeneratePdf
  PDF_OPTIONS = {
    ...
    :page_size   => "A4",
    # Defini o formato do layout como portrait (poderia ser landscape)
    :page_layout => :portrait,
    # Defini a margem do documento
    :margin      => [40, 75]
  }
  ...
  def self.agreement name, details, price
    ...
    Prawn::Document.new(PDF_OPTIONS) do |pdf|
      ...
      # Move mais 30 PDF points para baixo o curso
      pdf.move_down 30
      # Escreve o texto com os detalhes que o usuário entrou
      pdf.text "#{details}", :size => 12, :align => :justify, :inline_format => true
      # Move mais 30 PDF points para baixo o curso
      pdf.move_down 10
      # Adiciona o nome o nome com 12 PDF points, justify e com o formato inline (Observe que o <b></b> funciona para deixa em negrito)
      pdf.text "Com o cliente: <b>#{name}</b> por R$#{price}", :size => 12, :align => :justify, :inline_format => true
      # Muda de font para Helvetica
      pdf.font "Helvetica"
      # Inclue um texto com um link clicável (usando a tag link) no bottom da folha do lado esquerdo e coloca uma cor especifica nessa parte (usando a tag color)
      ...
      # Incluie em baixo da folha do lado direito a data e o numero da página usando a tag page
      ...
      # Gera no nosso psd e coloca na pasta public com o nome agreement.pdf
  ...
  def self.spending spendings
    ...
    # Formata os dados para gerar o gráfico (Não se preocupe com isso, apenas sáiba que nesse gáfico os dados de label precisar entrar como um hash)
    spending_labels = {}
    spendings.each_with_index {|s,i| spending_labels[i] = s[0].to_s}

    # Criar um objeto Gruff (gerado de gráfico)
    ...
    Prawn::Document.new(PDF_OPTIONS) do |pdf|
      ...
      # Inclue a imagem com o gráfico na escala 0.50 para diminuir pela metade a imagem
...
GeneratePdf.spending([["Publicidade", 10.0], ["Relações Públicas", 10.0], ["Gerência", 40.0], ["TI", 80.0]])
